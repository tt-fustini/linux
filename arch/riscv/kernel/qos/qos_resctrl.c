...
static int cbqri_apply_bw_config(struct cbqri_resctrl_dom *hw_dom, u32 closid,
				 enum resctrl_conf_type type, struct cbqri_config *cfg)
{
	return cbqri_apply_bc_field(hw_dom, closid,
				    cbqri_set_rbwb, cbqri_get_rbwb, cfg->rbwb);
}

/*
 * Sum Rbwb across all RCIDs except @skip_closid. Used to validate that a
 * pending Rbwb write will not violate CBQRI §4.5's sum(Rbwb) <= MRBWB
 * constraint. Caller must NOT hold ctrl->lock; this function takes it.
 */
static u64 cbqri_sum_other_rbwb(struct cbqri_controller *ctrl, u32 skip_closid,
				u32 max_rcid)
{
	u64 sum = 0;
	int i, err;
	...
	spin_lock(&ctrl->lock);
	for (i = 0; i < max_rcid; i++) {
		if (i == skip_closid)
			continue;
		err = cbqri_bc_alloc_op(ctrl, CBQRI_BC_ALLOC_CTL_OP_READ_LIMIT, i);
		if (err < 0)
			continue;
...
static int qos_init_domain_ctrlval(struct rdt_resource *r, struct rdt_ctrl_domain *d)
{
	...
	for (i = 0; i < hw_res->max_rcid; i++) {
		...
		case RDT_RESOURCE_RBWB:
			/*
			 * CBQRI §4.5: Rbwb >= 1, sum(Rbwb) <= MRBWB.
			 * RCID 0 gets the remaining budget so the initial sum
			 * equals MRBWB exactly.
			 */
			if (i == 0)
				...
				cfg.rbwb = 1;
			err = cbqri_apply_bw_config(dom, i, 0, &cfg);
			break;
		case RDT_RESOURCE_MWEIGHT:
			/*
