...
static int cbqri_probe_controller(struct cbqri_controller *ctrl)
{
	 ...
	 * Find the smallest mcid_count amongst all contrllers.
	 */
	if (max_rmid == 0)
	...
	if (ctrl->type == CBQRI_CONTROLLER_TYPE_CAPACITY) {
		...
		ctrl->cc.cache_level = ctrl->cache.cache_level;
...
static int qos_resctrl_add_controller_domain(struct cbqri_controller *ctrl)
{
	...
	if (ctrl->type == CBQRI_CONTROLLER_TYPE_CAPACITY) {
		...
		pr_warn("%s(): unknown resource %d", __func__, ctrl->type);
		err = -ENODEV;
		goto err_free_domain;
	}
...
err_unmap_controllers:
	/*
	 * TODO: unmap and release controller ioremap
	 * for each ctrl:
	 *	iounmap(ctrl->addr);
	 *	release_mem_region(ctrl->addr, ctrl->size);
	 */
