...
static int domain_setup_ctrlval(struct rdt_resource *r, struct rdt_ctrl_domain *d)
{
	...
	for (i = 0; i < hw_res->max_rcid; i++, dc++)
		*dc = 0; //TODO: resctrl_get_default_ctrl(r);
...
static int qos_resctrl_add_controller_domain(struct cbqri_controller *ctrl, int *id)
{
	...
	if (ctrl->ctrl_info->type == CBQRI_CONTROLLER_TYPE_CAPACITY) {
		...
		if (ctrl->alloc_capable) {
			...
			res->name = "MB";
			//res->schema_fmt = RESCTRL_SCHEMA_MBPS;
			res->ctrl_scope = RESCTRL_L3_CACHE;
			res->alloc_capable = ctrl->alloc_capable;
			/*
		...
		if (ctrl->mon_capable) {
			...
			internal_id++;
			list_add_tail(&mon_domain->hdr.list,
				      &mon_cbqri_res->resctrl_res.ctrl_domains);
...
err_free_mon_domain:
	...
	if (!mon_domain) {
		 ...
		 */
		kfree(container_of(mon_domain, struct cbqri_resctrl_dom, resctrl_ctrl_dom));
	}
