 ...
 * defined, so the cached value cannot be trusted after online. The startup
 * callback runs at CPUHP_AP_ONLINE_DYN, before CPUHP_AP_ACTIVE makes the CPU
 * schedulable for normal tasks, so the cache is invalidated before any such
 * task can run and its first __switch_to_srmcfg() is forced to write the CSR.
 * Using the same reset as the teardown callback is harmless symmetry only:
 * idle and per-CPU kthreads keep context switching while the CPU goes down
 * and clobber the sentinel with the CPU default, so it does not survive the
 * offline period.
 */
