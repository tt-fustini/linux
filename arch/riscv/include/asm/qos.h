...
static inline void __switch_to_srmcfg(struct task_struct *next)
{
	...
	if (thread_srmcfg != __this_cpu_read(cpu_srmcfg)) {
		...
		/*
		 * Zicsr 6.1.1: CSR writes are weakly ordered with respect
		 * to memory operations and classified as device-output
		 * for fence purposes.  Without an explicit fence, in-flight
		 * loads/stores from this hart may be tagged at the cache
		 * interconnect with the previous task's RCID/MCID even
		 * after the csrw retires.  ARM MPAM uses isb() for the
		 * analogous register; the RISC-V equivalent is `fence o,rw`
		 * to order the CSR write before subsequent memory accesses.
