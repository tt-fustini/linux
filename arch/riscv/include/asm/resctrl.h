 ...
 *
 * Each fast-path arch entry point below is the RISC-V realisation of the
 * generic contract documented in <linux/resctrl.h>.  Comments here describe
 * only the RISC-V-specific behaviour (srmcfg encoding, CBQRI controller
 ...
 * Only meaningful after qos_resctrl_setup() runs at late_initcall.
 */
 ...
 * monitoring event wired through resctrl (L3 occupancy or L3 MBM_TOTAL).
 */
 ...
 * to the CPU's default RCID/MCID for tasks tagged with
 * RISCV_RESCTRL_EMPTY_CLOSID.  Implements resctrl allocation rule 2
