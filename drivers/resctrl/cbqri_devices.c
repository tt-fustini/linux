 ...
 *
 * The caller supplies the cache id and level. The set of CPUs that share the
 * cache is not captured here. The resctrl glue derives domain membership from
 * each CPU's cacheinfo when it comes online, so a CPU that is offline at probe
 * time is still associated with its domain once it comes online.
 *
