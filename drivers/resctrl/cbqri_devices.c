...
static int cbqri_probe_cc(struct cbqri_controller *ctrl)
{
	...
	/*
	 * Resolve cache_size via cacheinfo, which SMP bring-up populates
	 * before this probe runs, so the lookup returns a real size here
	 * rather than a not-yet-ready 0. cpus_read_lock satisfies
...
int riscv_cbqri_register_controller(const struct cbqri_controller_info *info)
{
	...
	case CBQRI_CONTROLLER_TYPE_CAPACITY: {
		...
		/*
		 * cache_size stays 0 here: cacheinfo is not populated yet at
		 * acpi_arch_init time. It is resolved later during controller
		 * probe via get_cpu_cacheinfo_level(), by which point SMP
		 * bring-up has populated cacheinfo.
		 */
