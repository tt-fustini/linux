				...
				u64 *regp)
{
	...
	if (ret) {
		/*
		 * Record the fault for diagnostics. The flag is informational
		 * only: callers retry through this path on every op, so a
		 * transient stall self-heals once the controller responds.
		 */
		ctrl->faulted = true;
		return ret;
	}
