...
/*
 * Register a capacity controller whose cache topology is supplied directly
 * by the caller. A device-tree platform driver that already knows which CPUs
 * share the cache and at what level passes that in; there is no firmware
