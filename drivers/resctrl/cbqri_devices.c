...
static int cbqri_probe_cc(struct cbqri_controller *ctrl)
{
	 ...
	 * every cpu_mask member is offline, cache_size stays 0 and remains
	 * unavailable to monitoring features added in later patches.
	 */
	cpus_read_lock();
	...
	if (status == CBQRI_MON_CTL_STATUS_SUCCESS) {
		 ...
		 * cacheinfo did not give us a cache_size, leave mon_capable
		 * false so the file is not exposed at all rather than
 ...
 * counter.  Idempotent: callable more than once per BC; re-entries
 * keep accumulator state in use.
 *
...
/*
 * Return the single mon-capable BC, NULL if zero or more than one.
 * Mirrors MPAM's "one MSC, one L3" pairing: BC counters can only honestly
 * surface as L3 mbm_total_bytes if every memory request flows through
 * the same BC.
 */
