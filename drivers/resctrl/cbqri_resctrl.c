...
/*
 * fs/resctrl references the arch operations below at link time, but CBQRI
 * does not exercise them. The monitoring operations are reached only from
 * paths guarded by mon_capable, and CBQRI reports no monitoring capability
 * (resctrl_arch_mon_capable() returns false) and creates no monitoring
 * domains, so they are never invoked. resctrl_arch_get_io_alloc_enabled()
 * and resctrl_arch_io_alloc_enable() back the cache io_alloc allocation
 * feature, which CBQRI does not expose, so the stubs report it unavailable.
 */
 ...
 * resctrl only if they expose identical caps. resctrl publishes one cap set
 * per rid, but any of the level's controllers may service a given RCID.
 */
 ...
 * every globally-exposed cap. A level whose controllers disagree is dropped,
 * since resctrl exposes a single set of caps per rid, but the other level is
 * still picked. The first matching controller wins.
 */
static void cbqri_resctrl_pick_caches(void)
{
	/*
	 * A NULL cbqri_res->ctrl means either "nothing picked yet" or "picked
	 * then rejected". dropped[] records the latter so a later controller at
	 * the same level cannot revive a level that was thrown out.
	 */
	...
	list_for_each_entry(ctrl, &cbqri_controllers, list) {
		...
		if (cbqri_res->ctrl) {
			/*
			 * On a mismatch, drop the whole level rather than
			 * expose one controller's caps while programming
			 * another's. The other level, and any other QoS
			 * feature, is left untouched.
			 */
			if (!cbqri_cc_caps_agree(cbqri_res->ctrl, ctrl)) {
...
/*
 * Create and online a fresh ctrl_domain backing ctrl on resource res, seeded
 * with cpu and identified by dom_id. The domain is fully initialized and
 * brought online before it is published on the RCU ctrl_domains list. Caller
 * must hold cbqri_domain_list_lock and must have already verified that no
							...
							unsigned int cpu, int dom_id)
{
	...
	/*
	 * Publish only once the domain is fully initialized and online, sorted
	 * by id so the user-visible ordering is deterministic.
	 */
...
/*
 * Remove a CPU from the domain backing each cache level it belongs to. The
 * cache id reported by cacheinfo selects the domain directly. When the last
 * CPU of a domain goes away the domain is unlinked from the RCU list, drained,
 * then taken offline and freed. This is idempotent, so it also undoes a
 * partial online attach. Caller holds cbqri_domain_list_lock.
 */
static void cbqri_detach_cpu_from_all_ctrls(unsigned int cpu)
{
	...
	for (i = 0; i < ARRAY_SIZE(levels); i++) {
		struct cacheinfo *ci = get_cpu_cacheinfo_level(cpu, levels[i]);
		struct cbqri_resctrl_res *hw_res;
		struct rdt_ctrl_domain *domain;
		struct rdt_resource *res;
		int rid;
		...
		domain = cbqri_find_ctrl_domain(&res->ctrl_domains, ci->id);
		if (!domain)
			continue;
...
/*
 * Attach a CPU to the capacity controller backing each cache level it belongs
 * to. The cache id reported by cacheinfo for the online CPU selects both the
 * controller, by matching cache.cache_id, and the resctrl domain, which is
 * indexed by that id. On failure, detach the CPU from everything attached so
 * far. The cpuhp core does not run this state's offline teardown when its
 * startup fails, so a partial attach would otherwise leak into a domain
 * cpu_mask. Caller holds cbqri_domain_list_lock.
 */
static int cbqri_attach_cpu_to_all_ctrls(unsigned int cpu)
{
	...
	for (i = 0; i < ARRAY_SIZE(levels); i++) {
		...
		/* Find the controller governing this CPU's cache instance. */
		ctrl = NULL;
		list_for_each_entry(c, &cbqri_controllers, list) {
			...
			if (!c->alloc_capable)
				continue;
			if (c->cache.cache_level == levels[i] &&
		...
		domain = cbqri_find_ctrl_domain(&hw_res->resctrl_res.ctrl_domains,
						ci->id);
		if (domain) {
			cpumask_set_cpu(cpu, &domain->hdr.cpu_mask);
			continue;
		...
		domain = cbqri_create_ctrl_domain(ctrl, &hw_res->resctrl_res,
						  cpu, ci->id);
		if (IS_ERR(domain)) {
