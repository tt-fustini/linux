			     ...
			     u32 unused, u32 rmid, enum resctrl_event_id eventid)
{
	...
	/*
	 * See the rationale in resctrl_arch_rmid_read(). Same
	 * cbqri_mon_op() constraint, same MPAM parity.
	 */
	if (irqs_disabled())
		return;
	 ...
	 * mutex and frees the hw_dom on the last CPU offline. Hold the
	 * mutex across the ctrl->lock and BC paths so the hw_dom pointer
	 * stays valid until the MMIO sequence completes.
	 */
			   ...
			   void *arch_priv, u64 *val, void *arch_mon_ctx)
{
	...
	/*
	 * fs/resctrl picks smp_call_function_any() instead of
	 * smp_call_on_cpu() when the target domain has only nohz_full
	 * CPUs, so this can be invoked from a hardirq IPI handler.
	 * cbqri_mon_op() takes ctrl->lock (sleeping mutex) and polls
	 * BUSY for up to 1 ms, neither of which is safe under
	 * irqs_disabled(). Match MPAM's __read_mon() and return -EIO
	 * so the read fails predictably rather than deadlocking.
	 */
	if (irqs_disabled())
		return -EIO;
	 ...
	 * cbqri_detach_cpu_from_ctrl_domains(), which holds the same
	 * mutex and frees the hw_dom on the last CPU offline. Hold the
	 * mutex across the ctrl->lock / bc->lock paths so the hw_dom
	 * pointer stays valid until the MMIO sequence completes.
	 */
	mutex_lock(&cbqri_domain_list_lock);
	...
	case QOS_L3_OCCUP_EVENT_ID:
		...
		if (!err) {
			 ...
			 * so a non-power-of-2 ncblks doesn't truncate. Both
			 * terms fit in u64 with room to spare.
			 */
 ...
 * the minimum Rbwb of 1.  cbqri_probe_bc() already rejects a BC with
 * mrbwb < rcid_count so this subtraction cannot underflow today; the
 * WARN_ON_ONCE catches a future probe path that forgets the same check.
 */
...
void resctrl_arch_reset_all_ctrls(struct rdt_resource *r)
{
	...
	list_for_each_entry(d, &r->ctrl_domains, hdr.list) {
		...
		case RDT_RESOURCE_MB_MIN:
			 ...
			 * remaining budget. The hardware sum strictly
			 * decreases across the walk so it never exceeds
			 * MRBWB. The unchecked helper is used because the
			 * cache-based sum check would transiently fail while
			 * neighbor cache values are still being reset.
			 */
...
static int cbqri_init_domain_ctrlval(struct rdt_resource *r, struct rdt_ctrl_domain *d)
{
	...
	for (i = 0; i < hw_res->ctrl->rcid_count; i++) {
		...
		case RDT_RESOURCE_MB_MIN: {
			/*
			 * CBQRI section 4.5: Rbwb >= 1, sum(Rbwb) <= MRBWB.
			 * RCID 0 takes the remaining budget. The hardware
			 * sum strictly decreases through the walk and never
			 * exceeds MRBWB. The cache-based sum check is
			 * skipped because neighbor cache entries are still
			 * being seeded.
			 */
			u64 rbwb = (rcid == 0) ? cbqri_rcid0_rbwb(dom->hw_ctrl) : 1;
...
static int cbqri_resctrl_pick_caches(void)
{
	...
	list_for_each_entry(ctrl, &cbqri_controllers, list) {
		...
		if (!ctrl->alloc_capable) {
			/*
			 * A monitor-only CC is permitted by the CBQRI spec but
			 * not supported here: pick_caches keys off the allocation
			 * controller, and cbqri_attach_cpu_to_l3_mon() looks up
			 * hw_ctrl via the ctrl_domain. Decoupling those takes a
			 * separate series. Skip with a one-time warning so the
			 * limitation is visible.
			 */
			if (ctrl->mon_capable)
				pr_warn_once("CC @%pa: monitor-only controllers are not yet supported, skipping\n",
					     &ctrl->addr);
			continue;
		}
 ...
 * single BC with multiple L3 domains would make standard userspace
 * tools (which sum mbm_total_bytes across domains) overcount system
 * bandwidth by a factor equal to the L3 domain count.
 */
static void cbqri_resctrl_pick_counters(void)
{
	...
	/*
	 * Count distinct L3 cache_ids. Multiple CCs at the same cache_id
	 * (mismatched caps already rejected by pick_caches) collapse to
	 * one domain.
	 */
	list_for_each_entry(ctrl, &cbqri_controllers, list) {
		bool seen = false;
	...
	if (l3_count > 1) {
		pr_warn_once("multiple L3 domains (%u) detected, mbm_total_bytes disabled to avoid overcount\n",
			     l3_count);
		return;
	}
...
/*
 * Setup runs from cbqri_arch_late_init() at late_initcall, which is
 * strictly after the device_initcall_sync that drives cacheinfo's id <->
 * cpumask population.  cbqri_probe_cc()'s lazy cache_size fill therefore
 * has the data it needs without any explicit synchronization here.
 */
