...
static int cbqri_attach_cpu_to_all_ctrls(unsigned int cpu)
{
	...
	/*
	 * cbqri_controllers is mutated under cbqri_controllers_lock by
	 * riscv_cbqri_register_cc_dt(). Hold it across the walk, as the
	 * innermost lock, so a controller registered after boot cannot
	 * corrupt the iteration. The register path takes cbqri_controllers_lock
	 * as a leaf and never takes cbqri_domain_list_lock, so the nesting
	 * does not invert.
	 */
	mutex_lock(&cbqri_controllers_lock);
	list_for_each_entry(ctrl, &cbqri_controllers, list) {
