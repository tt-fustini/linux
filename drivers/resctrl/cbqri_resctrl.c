...
static inline struct cbqri_hw_ctrl *resctrl_to_cbqri_ctrl(struct resctrl_ctrl *c)
{
	return container_of(c, struct cbqri_hw_ctrl, r_ctrl);
}
 ...
 * (the DEF control), whose domains the monitoring paths consult to reach the
 * backing controller and the paired BC. NULL if the resource has no control.
 */
			     ...
			     u32 unused, u32 rmid, enum resctrl_event_id eventid)
{
	...
	case QOS_L3_MBM_TOTAL_EVENT_ID: {
		...
		ctrl = cbqri_first_ctrl(r);
		if (!ctrl)
			break;
		cd = cbqri_find_ctrl_domain(&ctrl->domains, d->hdr.id);
		if (!cd)
			break;
...
static void cbqri_resctrl_pick_counters(void)
{
	...
	if (l3->ctrl && l3->ctrl->mon_capable && cbqri_find_only_mon_bc())
		/* any_cpu: MMIO read, valid from any CPU. See QOS_L3_OCCUP. */
		resctrl_enable_mon_event(QOS_L3_MBM_TOTAL_EVENT_ID, true, 0, NULL);
}
...
static void cbqri_resctrl_accumulate_caps(void)
{
	 ...
	 * mon-capable controller with no exposed event (an L2 CC, or the BC
	 * when mbm_total_bytes is off) would otherwise leave max_rmid at
	 * U32_MAX and OOM rmid_init().
	 */
	exposed_mon_capable =
 ...
 * WGHT controls, each with its own per-prox-domain ctrl_domain list. Mirrors
 * the prior model where one BC was attached to two bandwidth resources.
 */
