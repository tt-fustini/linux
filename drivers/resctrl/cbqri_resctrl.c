...
/**
 * struct cbqri_resctrl_res - resctrl resource backed by CBQRI controllers
 * @ctrl:        representative controller for this rid. NULL if no probed
 *               controller could back it.  When multiple controllers share
 *               the same rid, one is chosen for filling rdt_resource caps
 *               while every matching controller still registers its own
 *               rdt_ctrl_domain.
 * @cdp_enabled: resctrl has enabled CDP for this rid (L2/L3 only).  Read by
 *               cbqri_apply_cache_config_dom() to decide one or two
 *               CONFIG_LIMIT ops per schemata write.  Mirrors
 *               rdt_hw_resource::cdp_enabled.
 ...
 * mon_capable.  They are stubbed here and replaced with real
 * implementations when the matching feature lands in a later patch.
 */
			     ...
			     u32 unused, u32 rmid, enum resctrl_event_id eventid)
{
	...
	/* Mirrors the resctrl_arch_rmid_read() guard: don't sleep with IRQs disabled. */
	if (irqs_disabled())
		return;
			    ...
			    u32 closid, enum resctrl_conf_type type)
{
	...
	case RDT_RESOURCE_L3: {
		...
		err = cbqri_read_cache_config(ctrl, closid, at, &cbm);
		if (err == 0)
			val = cbm;
		break;
	}
...
static int cbqri_resctrl_control_init(struct cbqri_resctrl_res *cbqri_res)
{
	...
	case RDT_RESOURCE_L3:
		...
		/*
		 * CBQRI has no external uncore agents claiming CBM bits.
		 * Mirrors the x86 convention where shareable_bits covers
		 * such agents. Here the full mask is available.
		 */
...
static int cbqri_attach_cpu_to_bw_ctrl(struct cbqri_controller *ctrl,
				       unsigned int cpu)
{
	struct cbqri_resctrl_res *hw_res = &cbqri_resctrl_resources[RDT_RESOURCE_MB_MIN];
...
static int cbqri_resctrl_setup(void)
{
	...
	if (!exposed_alloc_capable && !exposed_mon_capable) {
		pr_info("no resctrl-capable CBQRI controllers found\n");
		return -ENODEV;
	}
...
static int __init cbqri_arch_late_init(void)
{
	 ...
	 * inside it - via cbqri_resctrl_teardown().  Don't call
