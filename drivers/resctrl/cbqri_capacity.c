...
static int cbqri_capacity_probe(struct platform_device *pdev)
{
	...
	/*
	 * Resolve the cache id from cacheinfo. It is used as the resctrl
	 * domain id. The set of harts sharing the cache is not captured here.
	 * The resctrl glue derives domain membership from each hart's
	 * cacheinfo when it comes online.
	 *
