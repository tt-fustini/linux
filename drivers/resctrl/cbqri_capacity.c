 ...
 * cache node it governs; the driver registers it as the resctrl
 * cache-allocation resource for that cache.
 */
...
static int cbqri_capacity_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(dev->of_node, "riscv,cbqri-rcid",
				   &rcid_count);
	if (ret) {
		dev_err(dev, "missing riscv,cbqri-rcid property\n");
		return ret;
	}
	...
	/*
	 * Associate the controller with its cache instance via the kernel's
	 * cacheinfo, which records the DT cache node as the leaf fw_token. The
	 * matching leaf yields the resctrl domain id (cache id) and the set of
	 * harts that share the cache. cacheinfo only describes online CPUs, so
	 * the domain covers the harts online at probe time.
	 */
	cache_id = -1;
	cpus_read_lock();
...
static struct platform_driver cbqri_capacity_driver = {
	...
	.driver = {
		 ...
		 * for the life of the system; the core has no targeted
		 * unregister, mirroring the boot-time discovery model. Block
		 * unbind so userspace cannot leave a dangling controller.
		 */
...
/*
 * builtin_platform_driver() registers at device_initcall (level 6). The
 * controller platform device is populated from device tree by of_platform
 * earlier, so probe (and thus riscv_cbqri_register_cc_dt()) runs before the
 * CBQRI core's late_initcall (level 7) walks cbqri_controllers in
 * cbqri_arch_late_init().
 */
