 ...
 * fields in bits 63:32 on a future port to a 32-bit RISC-V.
 */
...
struct cbqri_controller {
	 ...
	 * cache before CONFIG_LIMIT. This avoids the silent-firmware-noop
	 * window that a READ_LIMIT round trip introduces: a silent no-op
	 * would otherwise leave stale per-RCID data in the unmodified field.
	 * rbwb_cache also lets cbqri_apply_rbwb() validate the
	 * sum(Rbwb) <= MRBWB invariant in O(rcid_count) memory accesses
	 * instead of O(rcid_count) READ_LIMIT round trips. Both arrays are
	 * allocated by cbqri_probe_bc() and NULL on capacity controllers.
	 */
	u16 *rbwb_cache;
	u8  *mweight_cache;
