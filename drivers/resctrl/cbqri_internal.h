...
/*
 * Capacity Controller (CC) MMIO register offsets.  Bandwidth Controller
 * (BC) offsets and monitoring offsets are added when the matching device
 * ops land.
 */
...
struct cbqri_controller {
	 ...
	 * preemption enabled - required for PREEMPT_RT.  All
	 * resctrl-arch entry points run in process context.
	 */
	struct mutex lock;
