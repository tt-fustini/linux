...
static ssize_t thread_qoscfg_value_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	old_thread_qoscfg = READ_ONCE(task->thread.srmcfg);
	trace_printk("DEBUG %s(): old_thread.srmcfg=0x%x", __func__, old_thread_qoscfg);

	trace_printk("DEBUG %s(): write value to thread.srmcfg=0x%x for pid=%d", __func__, value, task->pid);
	WRITE_ONCE(task->thread.srmcfg, value);

	thread_qoscfg = READ_ONCE(task->thread.srmcfg);
	trace_printk("DEBUG %s(): read thread.srmcfg=%d for pid=%d", __func__, thread_qoscfg, task->pid);
