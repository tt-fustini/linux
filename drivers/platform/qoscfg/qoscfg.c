...
static ssize_t thread_qoscfg_value_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	if (kstrtoint(buf, 10, &update) < 0)
		return -EINVAL;

	old = READ_ONCE(task->thread.srmcfg);
	WRITE_ONCE(task->thread.srmcfg, update);
	new = READ_ONCE(task->thread.srmcfg);

	//trace_printk("\n[task->thread.srmcfg is now 0x%2x]\n", task->thread.srmcfg);
	pr_err("DEBUG %s(): old: 0x%2x update: 0x%2x new: 0x%2x", __func__, old, update, new);
