...
``MB_MIN`` - per-group bandwidth reservation
  ...
  ``bandwidth_gran = 1`` are exposed in ``info/MB_MIN/``. A write that
  would push the sum over the budget is rejected with ``-EINVAL``,
  matching the existing schemata-write rejection convention. The
  read-sum-check-write sequence is serialised by the per-controller
  mutex so concurrent ``schemata`` writers cannot collectively push the
  sum past the budget. New groups are created at ``min_bandwidth`` to
  keep the sum invariant on the success path; ``mkdir`` itself returns
  ``-EINVAL`` once the budget is fully consumed (i.e. when
  ``sum(MB_MIN) + 1 > MRBWB``), since admitting a new group at the
  minimum reservation would exceed the controller budget.
...
* ``info/L3_MON/mon_features`` advertises ``mbm_total_bytes`` only.
  ...
  indistinguishable-from-total semantics and mislead userspace. The
  driver implements this suppression by simply not registering the
  ``QOS_L3_MBM_LOCAL_EVENT_ID`` in the resource's ``mon_features``
  bitmask - there is no framework-level policy enforcing the
  omission.
