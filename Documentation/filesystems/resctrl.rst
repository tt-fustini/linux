...
``MB_MIN`` - per-group bandwidth reservation
  ...
  write that would push the sum over the budget is rejected with
  ``-EINVAL``, matching the existing schemata-write rejection
  convention. The read-sum-check-write sequence is serialised by the
  per-controller mutex so concurrent ``schemata`` writers cannot
  collectively push the sum past the budget. All CLOSIDs are pre-seeded
  to ``min_bandwidth`` at domain creation so the sum invariant holds
  at every point; ``mkdir`` does not itself increase the sum, and a
  write to a freshly-created group's MB_MIN that exceeds the available
  headroom is what surfaces ``-EINVAL`` to userspace.
