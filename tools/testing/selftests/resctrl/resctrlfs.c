 ...
 *
 * Used for non-cache resources (e.g. RISC-V CBQRI bandwidth resources like
 * MB_MIN and MB_WGHT) whose domain IDs are not tied to CPU cache topology.
 * Requires resctrl to be mounted.
 *
 * Limitation: this returns the first domain listed in schemata regardless
 * of which CPU the caller intends to bind a workload to.  On multi-
 * proximity-domain systems where the workload runs on a CPU that is NOT
 * in the first listed domain, the caller will exercise the wrong
 * controller and may see zero or unrelated counter activity.  Tests that
 * need cross-domain coverage must enumerate domains explicitly and bind
 * the workload to a CPU in the chosen domain rather than relying on this
 * helper.  Single-domain platforms (the current CBQRI baseline) are
 * unaffected.
 *
