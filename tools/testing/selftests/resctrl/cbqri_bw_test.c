			   ...
			   const struct user_params *uparams)
{
	...
	/*
	 * The round-trip writes orig_root - 1 to root, so MRBWB <= 1 cannot
	 * be exercised (writing 0 is below min_bandwidth).  No SKIP path in
	 * the test runner. Pass with a diagnostic.
	 */
	if (orig_root < 2) {
...
out:
	/*
	 * Restore MB_WGHT before rmdir: the Mweight CSR persists past
	 * CLOSID free, so a leftover value would taint whoever next
	 * allocates the same CLOSID.  See analogous MB_MIN path below.
	 */
