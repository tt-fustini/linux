			   ...
			   const struct user_params *uparams)
{
	...
	/*
	 * The round-trip subtest below writes orig_root - 1 to root.  If
	 * MRBWB == 1 (orig_root == 1, the minimum legal value) that would
	 * write 0, which is below min_bandwidth and the kernel correctly
	 * rejects it; ditto for the (impossible-but-defensive) orig_root == 0
	 * case where the unsigned subtract wraps to UINT_MAX.  Return success
	 * with a diagnostic rather than failing with a confusing
	 * "write_schemata failed" message - the resctrl test runner does not
	 * support an in-test SKIP path and this case represents a runtime
	 * environmental constraint, not a failure of the kernel under test.
	 */
	if (orig_root < 2) {
		ksft_print_msg("MB_MIN round-trip not exercised: MRBWB=%u too small\n",
			       orig_root);
		return 0;
	}
...
out:
	/*
	 * Restore test-group MB_WGHT to the default before rmdir.  The
	 * CBQRI Mweight CSR persists past CLOSID free, so leaving the
	 * test group at the last-written 100 would taint the next
	 * process to allocate the same CLOSID with unexpected
	 * throttling.  See the analogous MB_MIN path below.
	 */
	snprintf(buf, sizeof(buf), "%u", CBQRI_MB_WGHT_MAX_BW);
	write_schemata(TEST_GROUP, buf, uparams->cpu, MB_WGHT_RESOURCE);
